...
function processRules(data: string): Rules {
	return [...data.matchAll(/((^\w+\b.*) bags contain (.*)\.)/gm)].reduce((a, b) => {
		(a as any)[b[2]] = b[3].split(', ').reduce((c, d) => {
			const types = [...d.matchAll(/((\d+) (.*) bags?)/g)][0];
...
function countBags(rules: Rules, colour: string): number {
	const rule = rules[colour];
	return Object.keys(rule).reduce((a: number, b: string) => {
		if (!rule[b]) return a;
		console.log(`${rule[b]} * ${b}`);
		return a + rule[b] + rule[b] * countBags(rules, b);
	}, 0);
}
